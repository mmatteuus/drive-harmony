...
export const api = {
  ...
  listCustomerDocuments: async (
    ...
    const url = new URL(`/api/customers/${encodeURIComponent(customerId)}/documents`, window.location.origin);
  ...
  sync: async (rootFolderId?: string) => {
    const token = getAccessToken();
    const url = new URL("/api/documents/sync", window.location.origin);
